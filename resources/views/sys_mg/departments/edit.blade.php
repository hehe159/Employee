    ...
    <div class="container">
        <div class="row">
            <div class="card col s12 m8 offset-m2 l8 offset-l2 xl8 offset-xl2 card-mt-15">
                <h4 class="center grey-text text-darken-2 card-title">Cập nhật Đơn vị</h4>
                    ...
                    <div class="row">
                        <form action="{{route('departments.update',$department->id)}}" method="POST">
                            ...
                            <i class="material-icons prefix">account_balance</i>
                                ...
                                <label for="dept_name">Tên Đơn vị</label>
                                <span class="{{$errors->has('dept_name') ? 'helper-text red-text' : ''}}">{{$errors->first('dept_name')}}</span>
                            </div>
                            @method('PUT')
                            @csrf()
                            <button type="submit" class="btn waves-effect waves-light mt-15 col s6 offset-s3 m4 offset-m4 l4 offset-l4 xl4-offset-xl4">Cập nhật</button>
                        </form>
                    </div>
